...
//    quick_shapes_.DrawAxes(ctm * S, view_matrix, proj_matrix);
    ...
    Color ant_skin = Color(0.72, 0.25, 0.25);
    Color ant_limb = Color(0.0, 0.0, 0.0);

    if (bone_name == "lhipjoint" || bone_name == "rhipjoint") {
      Point3 bone_pos =
        Point3(0.0, 0.0, 0.0) + skeleton_.BoneDirectionAndLength(bone_name);
        quick_shapes_.DrawLineSegment(ctm,
                                       view_matrix, proj_matrix, ant_limb,
                                       Point3(0.0, 0.0, 0.0), bone_pos, 0.01);
    }
    ...
    if (bone_name == "lowerback") {
      Matrix4 scale = Matrix4::Scale(Vector3(0.15, 0.375, 0.15));
      Matrix4 rotate = Matrix4::RotationZ(GfxMath::ToRadians(37.5))*
                       Matrix4::RotationX(GfxMath::ToRadians(37.5));
      Matrix4 translate = Matrix4::Translation(Vector3(0.0, -0.5, 0.0));
      quick_shapes_.DrawSphere(ctm * rotate * scale * translate,
                               view_matrix, proj_matrix, ant_skin);
    }
    if (bone_name == "upperback") {
      Matrix4 scale = Matrix4::Scale(Vector3(0.12, 0.122, 0.11));
      quick_shapes_.DrawSphere(ctm * scale,
                               view_matrix, proj_matrix, ant_skin);
    }
    if (bone_name == "thorax") {
        Matrix4 scale = Matrix4::Scale(Vector3(0.125, 0.125, 0.125));
        Matrix4 translate = Matrix4::Translation(Vector3(0.0, 0.3, 0.0));
        quick_shapes_.DrawSphere(ctm  * scale * translate,
                                 view_matrix, proj_matrix, ant_skin);

    }
    if (bone_name == "lowerneck" || bone_name == "upperneck") {
      Matrix4 scale = Matrix4::Scale(Vector3(0.025, 0.05, 0.025));
      quick_shapes_.DrawCylinder(ctm * scale,
                               view_matrix, proj_matrix, ant_skin);
    }
    if (bone_name == "head") {
      Matrix4 head = Matrix4::RotationZ(GfxMath::ToRadians(-45.0))*
                     Matrix4::RotationX(GfxMath::ToRadians(-45.0))*
                     Matrix4::Translation(Vector3(0.0, 0.1, 0.0))*
                     Matrix4::Scale(Vector3(0.12, 0.25, 0.12));
     quick_shapes_.DrawSphere(ctm * head,
                              view_matrix, proj_matrix, ant_skin);

     Matrix4 left_eye = Matrix4::Translation(Vector3(-0.04, 0.12, 0.03))*
                        Matrix4::Scale(Vector3(0.03, 0.03, 0.03));
     quick_shapes_.DrawSphere(ctm * left_eye,
                            view_matrix, proj_matrix, ant_limb);

     Matrix4 right_eye = Matrix4::Translation(Vector3(0.04, 0.12, 0.03))*
                         Matrix4::Scale(Vector3(0.03, 0.03, 0.03));
     quick_shapes_.DrawSphere(ctm * right_eye,
                             view_matrix, proj_matrix, ant_limb);

     Matrix4 left_tentacle = Matrix4::RotationZ(GfxMath::ToRadians(-45.0))*
                             Matrix4::RotationX(GfxMath::ToRadians(-45.0))*
                             Matrix4::Translation(Vector3(-0.04, 0.35, 0.0))*
                             Matrix4::Scale(Vector3(0.03, 0.2, 0.03));
     quick_shapes_.DrawCone(ctm * left_tentacle,
                             view_matrix, proj_matrix, ant_limb);

     Matrix4 right_tentacle = Matrix4::RotationZ(GfxMath::ToRadians(-45.0))*
                              Matrix4::RotationX(GfxMath::ToRadians(-45.0))*
                              Matrix4::Translation(Vector3(0.04, 0.35, 0.0))*
                              Matrix4::Scale(Vector3(0.03, 0.2, 0.03));
     quick_shapes_.DrawCone(ctm * right_tentacle,
                             view_matrix, proj_matrix, ant_limb);
    }
    ...
    Matrix4 child_root_transform =
        ctm * skeleton_.BoneSpaceToChildrenSpace(bone_name);
